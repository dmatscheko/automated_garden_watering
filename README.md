...
| `switch.garden_irrigation_pump_manual` | Manual well-pump control for e.g. a garden hose (only created if a pump is configured). Turning it **off** is blocked while a queue/backwash is active so the pump-first safety rule can't be broken manually. |
...
| `sensor.garden_irrigation_status` | `idle`, `pump_pressure`, `watering`, `backwash_pressure`, `backwash`. Slow-changing attributes only (queue, active zone, multiplier) so it doesn't flood the recorder. |
